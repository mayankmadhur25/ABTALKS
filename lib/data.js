...
/*
 * The four the brief names, and the only four offered in the reviewer's
 * switcher. "repaired" and "complete" are valid scenarios but are outcomes you
 * arrive at by finishing a flow, so they are reachable by URL rather than
 * listed as states to browse.
 */
...
export function getDayState(day, student) {
  const n = Number(day);
  // Repaired wins over dark. A night listed in both was repaired, and the
  // order of these two checks is the difference between a recovery reading as
  // a recovery and reading as a failure.
  if (student.repairedDays.includes(n)) return "done";
